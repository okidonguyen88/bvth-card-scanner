...
function FinalPreview({ croppedImages, onAddMore, onStartOver, onRemoveImage }) {
  const handlePrint = () => {
    if (window.electronAPI && window.electronAPI.printImage) {
      window.electronAPI.printImage(croppedImages).catch(err => {
          ...
          <html>
            <head>
              ...
              <style>
                ...
                body { padding: 16px; display: flex; flex-wrap: wrap; gap: 16px; }
                img { width: auto; height: 200px; display: block; object-fit: contain; }
              </style>
            </head>
            <body>
              ${croppedImages.map(src => `<div><img src="${src}" alt="Cropped ID Card" /></div>`).join('')}
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-xl shadow-2xl border border-slate-700 text-center">
      ...
      <p className="text-slate-300 mb-6">Bạn có thể thêm ảnh, xóa ảnh, hoặc in tất cả.</p>
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        {croppedImages.map((src, idx) => (
          <div key={idx} className="relative bg-slate-900/50 p-4 rounded-lg border border-slate-700 inline-block">
            <img src={src} alt={`Final Cropped ID ${idx + 1}`} className="max-w-xs md:max-w-sm rounded-md shadow-lg" />
            <button
